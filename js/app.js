...
const GORRA_CATS = ['Belicas', 'Dandys', 'Exclusivas', 'F1', 'Longhorns'];
const SECTIONS = [
  { id: 'gorras',  label: 'GORRAS',   categorias: GORRA_CATS },
  { id: 'tshirts', label: 'T-SHIRTS', categorias: ['T-shirts'] },
  { id: 'pants',   label: 'PANTS',    categorias: ['Pants'] },
];
...
function renderProductos() {
  ...
  SECTIONS.forEach(function(sec) {
    ...
    const gridEl = document.getElementById('grid-' + sec.id);

    let lista = productos.filter(p => sec.categorias.includes(p.categoria));

    if (filtroCategoria === 'gorras') {
      if (sec.id !== 'gorras') { secEl.classList.add('hidden'); return; }
    } else if (filtroCategoria !== 'all') {
      if (!sec.categorias.includes(filtroCategoria)) { secEl.classList.add('hidden'); return; }
      lista = lista.filter(p => p.categoria === filtroCategoria);
    }

    if (filtroBusqueda) {
      lista = lista.filter(p => p.nombre.toLowerCase().includes(filtroBusqueda.toLowerCase()));
    }

    if (lista.length === 0) { secEl.classList.add('hidden'); return; }

    secEl.classList.remove('hidden');
    totalMostrados += lista.length;
    gridEl.innerHTML = lista.map(renderCard).join('');
  });

  const emptyEl = document.getElementById('empty-global');
  const allHidden = SECTIONS.every(s => document.getElementById('section-' + s.id).classList.contains('hidden'));
  emptyEl.classList.toggle('hidden', !allHidden);

  info.textContent = totalMostrados + ' producto' + (totalMostrados !== 1 ? 's' : '');
}
